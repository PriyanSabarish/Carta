...
import configuration from 'D:/final project/basics/build/contracts/MyContract.json';
const Form = () => {
...
const contractAddress = configuration.networks['5777'].address;
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const tx = await contract.methods.storePatent(formData.name, formData.title, formData.description).send({
        from: accounts[0], // Replace with your account address
      });

      console.log('Transaction hash:', tx.transactionHash);
      alert('Patent submitted successfully!');

      setFormData({ name: '', title: '', description: '' }); // Clear form
    } catch (error) {
      console.error(error);
...
<form onSubmit={handleSubmit}>
      <label htmlFor="name">Inventor Name:</label>
      ...
      <input type="text" id="title" name="title" value={formData.title} onChange={(event) => setFormData({ ...formData, title: event.target.value })} required />

      <label htmlFor="description">Description:</label>
      <textarea id="description" name="description" value={formData.description} onChange={(event) => setFormData({ ...formData, description: event.target.value })} required></textarea>

      {/* Add fields for other struct properties here, following the same pattern */}
